...
- 禁止占用 5000 端口（主后端已使用），其他端口可用
- 避免遗留后台进程（用完记得清理），禁止使用 nohup 启动长期服务
...
6. **output_file 是必填字段**：write_file → 填要写入/修改的文件名（如 `init_db.py`）；analyze → 填报告文件名（如 `step_N.txt`）；execute_code → 填空字符串
